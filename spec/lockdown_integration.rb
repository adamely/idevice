...
describe Idev::LockdownClient do
  before :all do
    @lockdown = Idev::LockdownClient.new
  end

  after :all do
    @lockdown.destroy if @lockdown and not @lockdown.destroyed?
  end
  ...
  it "should raise an error starting a nonexistent lockdown service" do
    lambda{ @lockdown.start_service("nonexistent.service.garbage") }.should raise_error
  end
